...
Configuration options for the Cambia CFR+ training suite, based on YAML configuration files (e.g., `config.yaml`, `parallel.config.yaml`).
...
Parameters for Deep CFR training. These values are loaded from the YAML config and can be overridden via CLI options when running `cambia train deep`. See `src/cfr/deep_trainer.py:DeepCFRConfig` for the runtime configuration.

* **`hidden_dim`**:
  * Description: Width of the hidden layers in both the advantage and strategy networks.
  * Type: `integer`
  * Default: `256`
* **`dropout`**:
  * Description: Dropout rate applied between hidden layers during training.
  * Type: `float`
  * Default: `0.1`
* **`learning_rate`**:
  * Description: Learning rate for the Adam optimizer used to train both networks.
  * Type: `float`
  * Default: `0.001`
  * CLI Override: `--lr`
* **`batch_size`**:
  * Description: Mini-batch size when training networks on reservoir buffer samples.
  * Type: `integer`
  * Default: `2048`
  * CLI Override: `--batch-size`
* **`train_steps_per_iteration`**:
  * Description: Number of SGD steps per network per training phase. Each training step samples a mini-batch from the reservoir buffer.
  * Type: `integer`
  * Default: `4000`
  * CLI Override: `--train-steps`
* **`alpha`**:
  * Description: Iteration weighting exponent for the training loss. Each sample's loss is weighted by `(t+1)^alpha` where `t` is the iteration when the sample was collected. Higher values give more weight to recent samples.
  * Type: `float`
  * Default: `1.5`
  * CLI Override: `--alpha`
  * Examples: `1.0` (linear), `1.5` (default), `2.0` (quadratic)
* **`traversals_per_step`**:
  * Description: Number of external sampling traversals to run per training step, distributed across the worker pool.
  * Type: `integer`
  * Default: `1000`
  * CLI Override: `--traversals`
* **`advantage_buffer_capacity`**:
  * Description: Maximum number of samples in the advantage (regret) reservoir buffer. Uses Vitter's Algorithm R for uniform sampling.
  * Type: `integer`
  * Default: `2000000`
  * CLI Override: `--buffer-capacity` (sets both buffers)
* **`strategy_buffer_capacity`**:
  * Description: Maximum number of samples in the strategy reservoir buffer.
  * Type: `integer`
  * Default: `2000000`
  * CLI Override: `--buffer-capacity` (sets both buffers)
* **`save_interval`**:
