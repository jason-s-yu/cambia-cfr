...
# Global reference to the multiprocessing manager (if needed elsewhere, otherwise local)
# manager: Optional[multiprocessing.managers.SyncManager] = None # Managed locally now
    ...
    try:
        ...
        if queue_listener:
            ...
            try:
                ...
                logger.error("Error stopping queue listener: %s", ql_e)
        # Ensure the manager is shut down
        if manager:
            ...
            try:
                ...
                logger.error("Error shutting down manager: %s", mgr_e)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Context might have already been set
        print(f"DEBUG: Multiprocessing context already set? ({e})")
