...
# Import the specific dataclass and type aliases
...
# ReachProbDict is now defined in utils
...
class CFRTrainer:
    ...
    def __init__(self, config: Config):
        ...
        # Initialize with defaultdicts using the new types
        ...
        self.exploitability_results: List[Tuple[int, float]] = [] # Store (iteration, exploitability)
    ...
    def load_data(self, filepath: Optional[str] = None):
        ...
        if loaded:
             ...
             self.exploitability_results = loaded.get('exploitability_results', [])
    ...
    def save_data(self, filepath: Optional[str] = None):
        ...
        # (although joblib might handle dataclasses directly, tuple is safer)
        data_to_save = {
    ...
    def train(self, num_iterations: Optional[int] = None):
        ...
        for t in range(start_iteration, end_iteration):
            ...
            try:
                ...
                initial_state_str_for_log = str(game_state)
            ...
            if not initial_agent_states: continue
            ...
            last_exploitability = self.exploitability_results[-1][1] if self.exploitability_results else float('inf')
            exploitability_str = f"Expl: {last_exploitability:.4f}"
            if self.current_iteration % exploitability_interval == 0:
                 ...
                 if current_avg_strategy:
                      ...
                      exploitability_str = f"Expl: {exploit:.4f}"
                      logger.info(f"Exploitability calculated: {exploit:.4f} (took {time.time() - exploit_start_time:.2f}s)")
                 else:
                      logger.warning("Could not compute average strategy for exploitability calculation.")
                      exploitability_str = "Expl: N/A"
            ...
            if self.current_iteration % 100 == 0 or self.current_iteration == end_iteration:
                 ...
                 logger.info(f"Iter {self.current_iteration}/{end_iteration} | Last: {iter_time:.3f}s | "
                             f"Infosets: {infoset_count:,} | Avg Speed: {iters_per_sec:.2f} it/s | {utility_str} | {exploitability_str}")
        ...
        # Compute final average strategy and exploitability before saving
        ...
        if final_avg_strategy:
            ...
            exploit = self.analysis.calculate_exploitability(final_avg_strategy, self.config)
            self.exploitability_results.append((self.current_iteration, exploit))
            logger.info(f"Final exploitability: {exploit:.4f}")
        else:
    ...
    def _cfr_recursive(self, game_state: CambiaGameState, agent_states: List[AgentState], reach_probs: np.ndarray, iteration: int, action_log: List[Dict], depth: int) -> np.ndarray:
        ...
        try:
             ...
             # get_infoset_key returns the tuple structure
             base_infoset_tuple = current_agent_state.get_infoset_key()
             # Construct the InfosetKey dataclass instance
        ...
        if num_actions == 0:
             if not game_state.is_terminal():
                  ...
                  while local_undo_stack: local_undo_stack.popleft()() # Just undo, don't care about errors here
        ...
        if current_regrets is None or len(current_regrets) != num_actions:
             if current_regrets is not None: logger.warning(f"Iter {iteration}, P{player}, Depth {depth}, Infoset {infoset_key}: Regret dimension mismatch! Existing: {len(current_regrets)}, Expected: {num_actions}. Context: {current_context.name}. Re-initializing.")
        ...
        if current_strategy_sum is None or len(current_strategy_sum) != num_actions:
             if current_strategy_sum is not None: logger.warning(f"Iter {iteration}, P{player}, Depth {depth}, Infoset {infoset_key}: Strategy sum dimension mismatch! Existing: {len(current_strategy_sum)}, Expected: {num_actions}. Context: {current_context.name}. Re-initializing.")
        ...
        state_before_actions_str = str(game_state)
        ...
        for i, action in enumerate(legal_actions):
            ...
            if should_prune: logger.debug(f"Iter {iteration}, P{player}, Depth {depth}: Pruning action {action}"); continue # Skip recursion
            if action_prob < 1e-9 and not use_pruning: logger.debug(f"Iter {iteration}, P{player}, Depth {depth}: Skipping action {action} low prob."); continue
            ...
            state_delta: Optional[StateDelta] = None; undo_info: Optional[UndoInfo] = None; state_after_action_desc = "ERROR"; snap_log_before_action = list(game_state.snap_results_log)
            ...
            current_snap_log = game_state.snap_results_log # Get log *after* action
            observation = self._create_observation(None, action, game_state, player, current_snap_log)
    ...
    # --- _filter_observation, _create_observation remain unchanged ---
    ...
    def _create_observation(self, prev_state: Optional[CambiaGameState], action: Optional[GameAction], next_state: CambiaGameState, acting_player: int, snap_results: List[Dict]) -> AgentObservation:
         ...
         if next_state.pending_action and next_state.pending_action_player == acting_player:
              ...
              if isinstance(next_state.pending_action, ActionDiscard): drawn_card = pending_data.get("drawn_card")
                  ...
                  if "own_idx" in pending_data and "opp_idx" in pending_data and "card1" in pending_data and "card2" in pending_data:
                      ...
                      peeked_cards_dict = {(acting_player, pending_data["own_idx"]): pending_data["card1"], (opp_idx, pending_data["opp_idx"]): pending_data["card2"]}
         if isinstance(action, ActionAbilityPeekOwnSelect):
              hand = next_state.get_player_hand(acting_player)
              if hand and 0 <= action.target_hand_index < len(hand): peeked_cards_dict = {(acting_player, action.target_hand_index): hand[action.target_hand_index]}
         elif isinstance(action, ActionAbilityPeekOtherSelect):
              opp_idx = next_state.get_opponent_index(acting_player); opp_hand = next_state.get_player_hand(opp_idx)
              if opp_hand and 0 <= action.target_opponent_hand_index < len(opp_hand): peeked_cards_dict = {(opp_idx, action.target_opponent_hand_index): opp_hand[action.target_opponent_hand_index]}
         final_snap_results = next_state.snap_results_log
         obs = AgentObservation(
             acting_player=acting_player, action=action, discard_top_card=discard_top, player_hand_sizes=hand_sizes, stockpile_size=stock_size,
             drawn_card=drawn_card, peeked_cards=peeked_cards_dict, snap_results=final_snap_results, did_cambia_get_called=cambia_called, who_called_cambia=who_called,
    ...
    def compute_average_strategy(self) -> PolicyDict:
        ...
        for infoset_key, s_sum in self.strategy_sum.items():
             ...
             if r_sum > 1e-9:
                  ...
                  zero_reach_count += 1; normalized_strategy = np.ones(num_actions_in_sum) / num_actions_in_sum if num_actions_in_sum > 0 else np.array([])
        ...
        if zero_reach_count > 0: logger.warning(f"{zero_reach_count} infosets with zero reach.")
        ...
        if mismatched_dim_count > 0: logger.warning(f"{mismatched_dim_count} infosets with final dimension mismatch.")
        return avg_strategy
