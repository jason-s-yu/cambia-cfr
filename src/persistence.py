...
def load_agent_data(filepath: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        if os.path.exists(filepath):
            ...
            if 'regret_sum' not in loaded_data or 'strategy_sum' not in loaded_data or 'reach_prob_sum' not in loaded_data:
                 logger.warning(f"Loaded data from {filepath} missing expected keys. Check file integrity.")
