...
class KnownCardInfo:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.bucket, CardBucket):
            logger.warning(
                f"KnownCardInfo initialized with non-Enum bucket: {self.bucket}. Setting UNKNOWN."
            )
...
class AgentObservation:
    ...
    drawn_card: Optional[CardObject] = (
        None  # If observing player drew *or* action involved a drawn card
    )
    peeked_cards: Optional[Dict[Tuple[int, int], CardObject]] = (
        None  # {(player_idx, hand_idx): Card}
    )
    # Snap related info (Now detailed)
    snap_results: List[Dict[str, Any]] = field(
        default_factory=list
    )  # List of dicts from GameState.snap_results_log
    # General state info
...
class AgentState:
    ...
    def initialize(
        ...
        for i, card in enumerate(initial_hand):
            ...
            if known:
                logger.debug(
                    f"Agent {self.player_id} initial peek: Index {i} is {bucket.name}"
                )
        ...
        logger.debug(
            f"Agent {self.player_id} initialized (Turn {self._current_game_turn}). Own Hand({len(self.own_hand)}): { {k: v.bucket.name for k,v in self.own_hand.items()} }. Opponent({self.opponent_card_count}): { {k: v.name for k,v in self.opponent_belief.items()} }"
        )
    ...
    def update(self, observation: AgentObservation):
        ...
        if observation.current_turn < self._current_game_turn:
            logger.warning(
                f"Agent {self.player_id} received observation for past turn {observation.current_turn} (current: {self._current_game_turn}). Skipping."
            )
            return
            ...
            if snapper == self.player_id:  # Our snap attempt
                if success:
                    if removed_own_idx is not None:  # SnapOwn success
                        ...
                        logger.debug(
                            f"Agent {self.player_id} accounting for own removal at index {removed_own_idx} (SnapOwn)."
                        )
                    elif (
                        removed_opp_idx is not None
                    ):  # SnapOpponent success (Opponent loses card)
                        opponent_indices_removed.add(removed_opp_idx)
                        logger.debug(
                            f"Agent {self.player_id} accounting for opponent removal at index {removed_opp_idx} (SnapOpponent)."
                        )
                        # Opponent gains a card LATER when we move one (handled by expected count)
                elif penalty:
                    logger.debug(
                        f"Agent {self.player_id} accounting for own penalty draw ({num_penalty_cards} cards)."
                    )
                ...
                if success:
                    if removed_own_idx is not None:  # Opponent SnapOwn success
                        ...
                        logger.debug(
                            f"Agent {self.player_id} accounting for opponent removal at index {removed_own_idx} (Opponent SnapOwn)."
                        )
                    elif (
                        removed_opp_idx is not None
                    ):  # Opponent SnapOpponent success (We lose card)
                        own_indices_removed.add(removed_opp_idx)
                        logger.debug(
                            f"Agent {self.player_id} accounting for own removal at index {removed_opp_idx} (Opponent SnapOpponent)."
                        )
                        # We gain a card when opponent moves one (handled by expected count)
                elif penalty:
                    logger.debug(
                        f"Agent {self.player_id} accounting for opponent penalty draw ({num_penalty_cards} cards)."
                    )
        ...
        if action:
            if actor == self.player_id:
                if isinstance(action, ActionReplace):
                    ...
                    logger.debug(
                        f"## TEMP DEBUG ## Agent {self.player_id} handling Replace action for index {target_idx}. Drawn card in observation: {drawn_card_from_obs}"
                    )
                    if drawn_card_from_obs:
                        ...
                        if target_idx in self.own_hand:
                            ...
                            logger.debug(
                                f"Agent {self.player_id} updated own hand index {target_idx} to {drawn_bucket.name} after replace with {drawn_card_from_obs}."
                            )
                        else:
                            # This might happen if reconciliation failed, but should be less likely now
                            logger.warning(
                                f"Agent {self.player_id} Replace target index {target_idx} not found after reconciliation (Current indices: {list(self.own_hand.keys())}). State likely inconsistent."
                            )
                    else:
                        # This warning should no longer appear if the observation passing is fixed
                        logger.error(
                            "Replace action observed for self, but no drawn card in observation. Belief update failed."
                        )
                    ...
                    if observation.peeked_cards:
                        for (p_idx, h_idx), card in observation.peeked_cards.items():
                            if p_idx == self.player_id and h_idx in self.own_hand:
                                ...
                                logger.debug(
                                    f"Agent {self.player_id} ({action_type}) updated own index {h_idx} knowledge to {peeked_bucket.name}."
                                )
                            elif (
                                ...
                                action_type = (
                                    "KingLook"  # Only KingLook involves peeking opponent
                                )
                                logger.debug(
                                    f"Agent {self.player_id} ({action_type}) updated opponent index {h_idx} belief to {peeked_bucket.name}."
                                )
                    ...
                    if observation.peeked_cards:
                        for (p_idx, h_idx), card in observation.peeked_cards.items():
                            if (
                                ...
                                logger.debug(
                                    f"Agent {self.player_id} (PeekOther) updated opponent index {h_idx} belief to {peeked_bucket.name}."
                                )
                ...
                # --- Handle Swap Actions ---
                    ...
                    if own_idx in self.own_hand:
                        ...
                        logger.debug(
                            f"Agent {self.player_id} (BlindSwap) updated own index {own_idx} to UNKNOWN."
                        )
                    ...
                    if observation.peeked_cards:
                        ...
                        if len(involved_indices) == 2:
                            ...
                            for p_idx, h_idx in involved_indices:
                                if p_idx == self.player_id:
                                    own_idx = h_idx
                                elif p_idx == self.opponent_id:
                                    opp_idx = h_idx
                            ...
                            if own_idx != -1 and opp_idx != -1:
                                if own_idx in self.own_hand:
                                    ...
                                    logger.debug(
                                        f"Agent {self.player_id} (KingSwap) updated own index {own_idx} to UNKNOWN."
                                    )
                                # Trigger decay/unknown for opponent's involved index
                                self._trigger_event_decay(
                                    target_index=opp_idx,
                                    trigger_event="swap (king)",
                                    current_turn=self._current_game_turn,
                                )
                            else:
                                logger.warning(
                                    "Could not determine indices for King Swap update from peeked_cards."
                                )
                        else:
                            logger.warning(
                                "Incorrect peeked_cards format for King Swap update."
                            )
                    else:
                ...
                if isinstance(action, ActionReplace):
                    ...
                    )  # Index in opponent's hand involved in swap
                    our_idx = (
                        action.opponent_hand_index
                    )  # Index in our hand involved in swap
                    ...
                    if our_idx in self.own_hand:
                        ...
                        logger.debug(
                            f"Agent {self.player_id} updated own index {our_idx} to UNKNOWN due to opponent BlindSwap."
                        )
                    ...
                    if observation.peeked_cards:
                        ...
                        for p_idx, h_idx in observation.peeked_cards.keys():
                            if p_idx == self.player_id:
                                our_involved_idx = h_idx
                            elif p_idx == self.opponent_id:
                                opp_involved_idx = h_idx
                        ...
                        if our_involved_idx != -1 and our_involved_idx in self.own_hand:
                            ...
                            logger.debug(
                                f"Agent {self.player_id} updated own index {our_involved_idx} to UNKNOWN due to opponent KingSwap."
                            )
                        # Trigger decay for opponent's involved index
                        if opp_involved_idx != -1:
                            self._trigger_event_decay(
                                target_index=opp_involved_idx,
                                trigger_event="swap (king, opponent)",
                                current_turn=self._current_game_turn,
                            )
                        elif our_involved_idx == -1 and opp_involved_idx == -1:
                            logger.debug(
                                "Opponent King Swap observed, but couldn't identify involved indices from peeked_cards."
                            )
                    else:
                        ...
                        logger.debug(
                            "Opponent King Swap observed without peek info, cannot update specific beliefs."
                        )
    ...
    def _rebuild_hand_state(
        ...
        for i, item in enumerate(current_items):
            if i < expected_count:
                ...
                logger.error(
                    f"Rebuild {player_desc} Hand: More items ({len(current_items)}) than expected ({expected_count}) after removal. Discarding extra."
                )
        ...
        while len(new_dict) < expected_count:
            ...
            if is_own_hand:
                ...
                logger.error(
                    "_rebuild_hand_state called for opponent belief. Use _rebuild_belief_state."
                )
        ...
        if len(new_dict) != expected_count:
            player_desc = "Own" if is_own_hand else "Opponent"
            logger.error(
                f"FATAL: {player_desc} hand final rebuild failed. Expected {expected_count}, got {len(new_dict)}. Indices: {sorted(list(new_dict.keys()))}"
            )
    ...
    def _rebuild_belief_state(
        ...
        for idx, belief in sorted(original_belief_dict.items()):
            if idx not in removed_indices:
                ...
                if idx in original_last_seen_dict:
                    ...
                    current_last_seen_mapping[belief] = original_last_seen_dict[
                        idx
                    ]  # Note: This assumes beliefs are unique enough or last seen applies to the bucket
        ...
        for i, belief in enumerate(current_belief_items):
            if i < expected_count:
                ...
                logger.error(
                    f"Rebuild Opponent Belief: More items ({len(current_belief_items)}) than expected ({expected_count}) after removal. Discarding extra."
                )
                break
        ...
        if len(new_belief_dict) != expected_count:
            logger.error(
                f"FATAL: Opponent belief final rebuild failed. Expected {expected_count}, got {len(new_belief_dict)}. Indices: {sorted(list(new_belief_dict.keys()))}"
            )
    ...
    def _estimate_stockpile(self, stock_size: int) -> StockpileEstimate:
        ...
        total_cards = 52 + (
            self.config.cambia_rules.use_jokers
            if hasattr(self.config, "cambia_rules")
            else 2
        )
    ...
    def _estimate_game_phase(
        ...
        if stock_cat == StockpileEstimate.EMPTY:
            return GamePhase.LATE  # Empty is always late
        if stock_cat == StockpileEstimate.LOW:
            return GamePhase.LATE  # Low stockpile implies late
        if stock_cat == StockpileEstimate.MEDIUM or current_turn >= turn_threshold_late:
            return GamePhase.MID  # Medium stock or many turns
    ...
    def _trigger_event_decay(
        ...
        if target_index in self.opponent_belief:
            ...
            if (
                ...
                logger.debug(
                    f"Agent {self.player_id} decaying Opponent[{target_index}] belief from {current_belief.name} to {decayed_category.name} due to {trigger_event}."
                )
            elif isinstance(current_belief, DecayCategory):
                logger.debug(
                    f"Agent {self.player_id} decay trigger skipped for Opponent[{target_index}]: Already decayed to {current_belief.name}."
                )
            else:  # Already UNKNOWN
                logger.debug(
                    f"Agent {self.player_id} decay trigger skipped for Opponent[{target_index}]: Already UNKNOWN."
                )
    ...
    def _apply_time_decay(self, current_turn: int):
        ...
        for idx, last_seen in self.opponent_last_seen_turn.items():
            ...
            if current_turn - last_seen >= self.time_decay_turns:
                ...
                if (
                    idx in self.opponent_belief
                    and isinstance(self.opponent_belief[idx], CardBucket)
        ...
        for idx in indices_to_decay:
            ...
            logger.debug(
                f"Agent {self.player_id} applying time decay for Opponent[{idx}] from {current_belief.name} to {decayed_category.name}."
            )
    ...
    def get_infoset_key(self) -> InfosetKey:
        ...
        for i in range(self.opponent_card_count):
            belief = self.opponent_belief.get(
                i, CardBucket.UNKNOWN
            )  # Get belief for contiguous index i
    ...
    def __str__(self) -> str:
        ...
        return (
            f"AgentState(P{self.player_id}, GT:{self._current_game_turn}, Phase:{self.game_phase.name}, "
